...
def calc(calc_result, idx) :
    global max_value
    global min_value
    if(idx >= num_count) :
        max_value = max(max_value, calc_result)
        min_value = min(min_value, calc_result)
        return

    calc_value = num_list[idx];
    if (operator_list[0] > 0 ) :
        operator_list[0] = operator_list[0] - 1
        calc(calc_result+calc_value, idx+1)
        operator_list[0] = operator_list[0] + 1

    if (operator_list[1] > 0 ) :
        operator_list[1] = operator_list[1] - 1
        calc(calc_result-calc_value, idx+1)
        operator_list[1] = operator_list[1] + 1

    if (operator_list[2] > 0 ) :
        operator_list[2] = operator_list[2] - 1
        calc(calc_result*calc_value, idx+1)
        operator_list[2] = operator_list[2] + 1

    if (operator_list[3] > 0 ) :
        operator_list[3] = operator_list[3] - 1
        if(calc_result < 0) :
            calc(-(abs(calc_result)//calc_value), idx+1)
        else :
            calc((calc_result)//calc_value, idx+1)
        operator_list[3] = operator_list[3] + 1
